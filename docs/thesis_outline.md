...
### 5.3 Final Full-Corpus Result (session 2026-08-13, supersedes §5.2's headline numbers)

- §5.2's numbers are from an early checkpoint (`checkpoints_mbert_amd`, pre-backfill corpus, ~2026-08-05). Between then and this final run the corpus grew substantially (CDLI-catalog + eBL photo/text backfill across weak provenience/period/genre/language classes, plus a curated showcase set of canonical literary texts — see `docs/data_layout.md`), and the deprecated from-scratch custom-transformer track (§4.2, §5.3 sign-level) was removed entirely as unproductive relative to mBERT at comparable training cost (§6, "Revised track pairing", resolved below). The number to actually cite as this project's final restoration result is the one below, not §5.2.
  — This work: `checkpoints_final_text/training_history_20260813_051735.json`, `docs/final_results.md`.

- Final text-only mBERT (`checkpoints_final_text`, 24 epochs, `context_char_max=850`, full post-backfill `documents` corpus): MLM MRR 0.797, Hit@5 0.866, Hit@3 0.832, top-1 0.738 (vs. Lazar et al. 2021's MRR .83 / Hit@5 .89 — 96.0% and 97.3% respectively, closer than the earlier §5.2 checkpoint's 94.5%/96.9%, on a larger and more class-balanced corpus).
  — Lazar et al. 2021, Table 2, p.4686.
  — This work: as above.

- Metadata heads, same checkpoint: period macro-F1 0.861, genre 0.854, language 0.780, provenience 0.725 — all substantially above the §5.2 checkpoint's figures (genre 0.747→0.854, period 0.645→0.861, language 0.634→0.780, provenience 0.557→0.725), consistent with the backfill's stated goal of bringing weak classes up toward the well-represented ones rather than just adding volume indiscriminately.
  — This work: as above.

### 5.4 Provenience Vision Head — Results (session 2026-08-13)

Following §6's original plan (image conditioning restricted to the geography/provenience head, by direct analogy with Aeneas's own architecture), this project implemented and empirically validated an image-conditioned `provenience_head`: a ResNet18 (ImageNet-initialized, fine-tuned jointly with the rest of the model — `vision_init=finetune`) feature, LayerNorm'd and concatenated with the BERT `[CLS]` embedding, matching Aeneas's own `x_img_norm` mechanism exactly.
  — Assael et al. 2025, Methods, p.148 (ResNet-8, concatenation with textual embeddings, geography-head-only scoping).
  — This work: `src/training/train_mbert.py` (`MBertMultiTask`, `IMG_TRANSFORM_TRAIN`/`_EVAL`).

- **Result**: provenience macro-F1 0.725 (text-only) → 0.766 (vision), reproduced in two independent runs (0.766, 0.764 — 0.002 apart). The effect clears the noise floor cleanly: across four runs where provenience was *not* image-conditioned, its macro-F1 ranged only [0.721, 0.736] — no overlap at all with the two conditioned runs.
  — This work: `docs/final_results.md` (full run table); `docs/ablation_runs/provenience_run2/`.

- **Controlled ablation, not just a copied architectural choice**: this project independently re-tested whether period/genre also benefit from image conditioning (motivated by this session's own pipeline fixes — ImageNet normalization and LayerNorm scale-matching that Aeneas's own design already had but this project's first vision pilot lacked — raising the question of whether the original null result for period/genre had been a pipeline artifact rather than a genuine absence of visual signal). Result: no effect for either head, evaluated against a noise floor established the same way as provenience's positive result (the `language` head, which is never image-conditioned in any run, serves as a running noise control — its macro-F1 swings by up to 0.05 between otherwise-identical runs, larger than either provenience's or genre's apparent movement in a naively single-run reading). One apparent genre signal (macro-F1 0.865 in a joint period+genre run) did not reproduce once genre was tested in isolation (0.860, back inside the noise band) — plausibly leakage through the shared vision backbone with period rather than an independent genre signal, a caution worth stating explicitly given how easy it would have been to over-read the joint run alone.
  — This work: `docs/final_results.md` (full ablation table and noise-floor reasoning); `docs/ablation_runs/{genre_only,period_only,period_genre}/`.

- This independently reproduces Aeneas's own finding (vision helps geography, not date; Aeneas did not test genre specifically) on a completely different, much smaller corpus (~5.3k vs. ~176k images) and a different underlying vision architecture (ResNet18/finetune vs. their ResNet8/from-scratch), which strengthens rather than merely repeats the original claim.
...
- **Multimodal (image-conditioned) head — implemented and validated, see §5.4 (was planned future work as of the 2026-08-05 draft of this outline).** Aeneas's own design was the direct precedent going in: only the geographical-attribution head receives visual input; restoration explicitly excludes it to prevent information leakage; the dating head excludes it because ablations showed no measurable gain. This project independently re-ran that same three-way question (period/genre/provenience) after this session's own pipeline fixes and reached the identical conclusion on its own, much smaller corpus — see §5.4 for the controlled ablation and noise-floor methodology, not just the citation.
...
- **Track pairing — resolved by removing the sign-level track, not by reconciling it.** The original plan (see the 2026-08-05 draft of this section) worried about pairing a future image head with the sign-level (glyph) track vs. the mBERT (transliteration) track, since CuneiML images correspond physically to glyphs. This tension was resolved this session by deleting the from-scratch custom-transformer track entirely (`src/training/{train,model,tokenizer,test}.py` and the sign-level embedding-search inference tooling) after it remained unproductive relative to mBERT at comparable training cost — not by finding a principled reason to still prefer glyphs. Worth stating plainly in the write-up as a scope reduction, not a silent substitution: the thesis's restoration results are mBERT-only.
  — This work: commit `deebe5f`, session 2026-08-12 refactor.

- **Image bounding-box quality — resolved via full manual review, not just flagged.** The 2026-08-05 draft of this section reported a 58% pass rate on a 24-sample audit of CuneiML's automated bounding boxes and recommended filtering before training any vision head. This session completed a full manual review of every collected candidate image (`review_bboxes_gui.py`, decisions logged to `data/bbox_corrections.jsonl`) rather than a statistical filter — every image actually used by `checkpoints_final_vision` has a human-drawn or human-confirmed box (`finalize_vision_crops.py`), so the 58%-reliability concern no longer applies to the reported vision results. It remains worth reporting as a data-quality finding about the source (CuneiML's self-reported 97% vs. this project's measured 58% on its own sample is a real, citable discrepancy), just not as an open risk to the results themselves anymore.
...
*Draft (session 2026-08-13) — expand into prose, this is talking points only.*

- mBERT fine-tuning reaches 96% of Lazar et al. 2021's reported MLM restoration quality (MRR 0.797 vs. 0.83) on a larger, more class-balanced corpus than either Lazar or Fetaya used, confirming their own finding that mBERT's cross-lingual pretraining transfers usefully to Akkadian even without a bespoke monolingual model (§2.3.1).
- The from-scratch custom-transformer track was abandoned as unproductive relative to mBERT at comparable training cost — a real result (a negative one) about this specific low-resource regime, not just an implementation detail; worth stating explicitly rather than omitting the track from the write-up entirely.
- Joint metadata classification (period/genre/language/provenience) benefits substantially from proper loss weighting relative to the restoration objective (§4.3) and from the corpus backfill targeting weak classes specifically (§5.3) — both are "the data/training recipe matters as much as the architecture" findings, in the same spirit as Aeneas's own emphasis on data engineering.
- The provenience vision head is this project's one genuinely new empirical contribution beyond replicating prior work: not just "Aeneas restricted vision to geography, so we did too," but an independent re-derivation of that same conclusion via a controlled, noise-floor-aware ablation on a dataset two orders of magnitude smaller and a different vision backbone (§5.4) — the fact that it reproduces Aeneas's finding rather than contradicting it is itself evidence that the geography-specific visual signal (tablet material/shape/photographic convention correlating with findspot) is a real, architecture-independent property of the data, not an artifact of their specific pipeline.
